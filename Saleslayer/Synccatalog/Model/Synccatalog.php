...
class Synccatalog extends \Magento\Framework\Model\AbstractModel{
    ...
    private function loadStoreViewIds($connector_id) {
        ...
        if (!is_null($store_view_ids)){
            ...
            if ($this->store_view_ids === false) $this->store_view_ids = array();
    ...
    private function load_format_configurable_attributes ($connector_id) {
        ...
        if (!is_null($format_configurable_attributes)){
            ...
            if ($this->format_configurable_attributes === false) $this->format_configurable_attributes = array();
    ...
    private function connect_saleslayer ($connector_id, $secretKey) {
        ...
        $this->slDebuger->debug('Connecting with API... (last update: '.$last_date_update.')');
    ...
    public function store_sync_data ($connector_id, $last_sync = null) {
        ...
        if ($get_response_table_data) {
            ...
            if ($this->sl_DEBBUG > 1) $this->slDebuger->debug('##### time_all_store_process: ', 'timer', (microtime(1) - $time_process));
    ...
    */
    private function prepare_category_data_to_store ($arrayCatalogue) {
        ...
        if (!empty($arrayCatalogue)){
            ...
            if ($this->sl_DEBBUG > 1) $this->slDebuger->debug('### time_reorganize_categories: ', 'timer', (microtime(1) - $time_ini_reorganize));
    ...
    */
    private function prepare_product_data_to_store($arrayProducts){
        ...
        if (!isset($schema['fields'][$this->product_field_name])){
            ...
            $this->slDebuger->debug('## Error. Product name field must be defined in order to synchronize information.');
        ...
        if (!isset($schema['fields'][$this->product_field_sku])){
            ...
            $this->slDebuger->debug('## Error. Product SKU field must be defined in order to synchronize information.');
            $this->stop_storage_process = true;
            return false;
    ...
    */
    private function prepare_product_format_data_to_store($arrayFormats){
        ...
        if (!isset($schema['fields'][$this->format_field_name])){
            ...
            $this->slDebuger->debug('## Error. Product format name field must be defined in order to synchronize information.');
            $this->stop_storage_process = true;
            return false;
        ...
        if (!isset($schema['fields'][$this->format_field_sku])){
            ...
            $this->slDebuger->debug('## Error. Product format SKU field must be defined in order to synchronize information.');
            $this->stop_storage_process = true;
            return false;
    ...
    private function sync_category_data_db($category, $store_view_ids){
        ...
        if (!empty($sl_category_image_data_to_sync)){
            ...
            $this->slDebuger->debug(" > SL category image data to sync: ".print_r($sl_category_image_data_to_sync,1));
    ...
    private function check_product_db($product){
        ...
        if ($product['data'][$this->product_field_name] == ''){
            ...
            $this->slDebuger->debug('## Error. Product with SL ID: '.$sl_id.' has no name.');
            return false;
        ...
        if ($product['data'][$this->product_field_sku] == ''){
            ...
            $this->slDebuger->debug('## Error. Product with name: '.$product['data'][$this->product_field_name].' and SL ID: '.$sl_id.' has no SKU.');
            return false;
        ...
        if (empty($this->sl_product_mg_category_ids)){
            $this->slDebuger->debug('## Error. Product '.$product['data'][$this->product_field_name].' with SL ID '.$product[$this->product_field_id].' has no valid categories.');
            return false;
        }
    ...
    public function sync_stored_product_images_db($images_data, $item_index){
        ...
        if (isset($images_data['existing_images_to_delete']) && !empty($images_data['existing_images_to_delete'])){
            ...
            foreach ($images_data['existing_images_to_delete'] as $id_image_to_delete => $image_to_delete){
                ...
                //         $this->slDebuger->debug('Deleting image types: '.print_r($image_types,1));
                //         $this->setProductImageTypes($item_id, 'catalog_product_entity', $image_data_to_remove, $this->product_entity_type_id);
                //         if ($this->sl_DEBBUG > 2) $this->slDebuger->debug('# time_image_delete_types: ', 'timer', (microtime(1) - $time_ini_image_delete_types));
    ...
    */
    ...
    private function check_format_db($format){
        ...
        if ($format['data'][$this->format_field_name] == ''){
            ...
            $this->slDebuger->debug('## Error. Product format with SL ID: '.$sl_id.' has no name.');
            return false;
        ...
        if ($format['data'][$this->format_field_sku] == ''){
            ...
            $this->slDebuger->debug('## Error. Product with name: '.$format['data'][$this->format_field_name].' and SL ID: '.$sl_id.' has no SKU.');
            return false;
    ...
    //                             // $this->slDebuger->debug('Count total categories: '.count($modified_data));
    ...
    //                             // $this->slDebuger->debug('Count total products: '.count($modified_data));
    ...
    //                             // $this->slDebuger->debug('Count total product formats: '.count($modified_data));
    ...
    //                                 $this->slDebuger->debug('@@@ category already unlinked SL id: '.$category_saleslayerid.' SL comp_id: '.$category_saleslayercompid);
    //                                 $this->slDebuger->debug('@@@ MG unlinked id: '.$unlinked_items[$category_saleslayercompid]['category'][$category_saleslayerid].' MG new unlink id: '.$category['entity_id']);
    ...
    //                             $this->slDebuger->debug('@@@ category unlink id: '.$category['entity_id'].' name: '.$category['name']);
    //                             $this->slDebuger->debug('@@@ category unlink category_saleslayerid: '.print_r($category_saleslayerid,1));
    //                             $this->slDebuger->debug('@@@ category unlink category_saleslayercompid: '.print_r($category_saleslayercompid,1));
    ...
    //                             $this->slDebuger->debug('## Error. Unlinking category: '.$e->getMessage());
    ...
    //                                 $this->slDebuger->debug('## Error. Reorganizing category tree: '.$e->getMessage());
    ...
    //                                     $this->slDebuger->debug('@@@ format already unlinked SL format_id: '.$product_saleslayerformatid.' SL comp_id: '.$product_saleslayercompid.' SL prod_id: '.$product_saleslayerid);
    //                                     $this->slDebuger->debug('@@@ MG unlinked id: '.$unlinked_items[$product_saleslayercompid]['format'][$product_saleslayerformatid].' MG new unlink id: '.$product['entity_id']);
    //                                 }
    ...
    //                                     $this->slDebuger->debug('@@@ product already unlinked SL id: '.$product_saleslayerid.' SL comp_id: '.$product_saleslayercompid);
    //                                     $this->slDebuger->debug('@@@ MG unlinked data: '.print_r($unlinked_items[$product_saleslayercompid]['product'][$product_saleslayerid],1));
    //                                     $this->slDebuger->debug('@@@ MG new unlink id: '.$product['entity_id']);
    ...
    //                             $this->slDebuger->debug('@@@ product unlink id: '.$product['entity_id'].' sku: '.$product['sku'].' name: '.$product['name']);
    //                             $this->slDebuger->debug('@@@ product unlink product_saleslayerid: '.print_R($product_saleslayerid,1));
    //                             $this->slDebuger->debug('@@@ product unlink product_saleslayercompid: '.print_r($product_saleslayercompid,1));
    //                             $this->slDebuger->debug('@@@ product unlink product_saleslayerformatid: '.print_r($product_saleslayerformatid,1));
    ...
    //                             $this->slDebuger->debug('### Error. Unlinking product: '.$e->getMessage());
    ...
    //                                 $this->slDebuger->debug('@@@ duplicated category unlink id: '.$dup_category->getId().' name: '.$dup_category->getName());
    //                                 $this->slDebuger->debug('@@@ duplicated category unlink category_saleslayerid: '.print_r($dup_category->getSaleslayerId(),1));
    //                                 $this->slDebuger->debug('@@@ duplicated category unlink category_saleslayercompid: '.print_r($dup_category->getSaleslayerCompId(),1));
    ...
    //                                 $this->slDebuger->debug('### Error. Unlinking duplicated category: '.$e->getMessage());
    ...
    //                                 $this->slDebuger->debug('@@@ duplicated product unlink id: '.$dup_product->getId().' sku: '.$dup_product->getSku().' name: '.$dup_product->getName());
    //                                 $this->slDebuger->debug('@@@ duplicated product unlink product_saleslayerid: '.print_R($dup_product->getSaleslayerId(),1));
    //                                 $this->slDebuger->debug('@@@ duplicated product unlink product_saleslayercompid: '.print_r($dup_product->getSaleslayerCompId(),1));
    //                                 $this->slDebuger->debug('@@@ duplicated product unlink product_saleslayerformatid: '.print_r($dup_product->getSaleslayerFormatId(),1));
    ...
    //                                 $this->slDebuger->debug('### Error. Unlinking duplicated product: '.$e->getMessage());
    //                             }
        ...
        if (!empty($sl_multiconn_table_data)){
            ...
            foreach ($sl_multiconn_table_data as $sl_multiconn_reg) {
                ...
                if ($sl_multiconn_reg['sl_connectors'] !== ''){
                    ...
                    if ($sl_multiconn_reg['sl_connectors'] === false) $sl_multiconn_reg['sl_connectors'] = array();
   ...
   public function checkFilesLogs(){
    ...
    private function processResponse($get_response_table_data, $connector_id, $sync_params){
        ...
        if ($get_response_table_data_new = $this->checkFrmAsPrd($get_response_table_data)){

            $get_response_table_data = $get_response_table_data_new;
    ...
    private function storeModifiedCategories($modified_data, $sync_params){
        ...
        $this->slDebuger->debug('Total count of modified categories to store: '.$categories_to_sync_count);
        if ($this->sl_DEBBUG > 1) $this->slDebuger->debug('Modified categories data to store: '.print_r($modified_data,1));
    ...
    private function storeModifiedProducts($modified_data, $sync_params){
        ...
        $this->slDebuger->debug('Total count of modified products to store: '.$product_to_sync_count);
        if ($this->sl_DEBBUG > 1) $this->slDebuger->debug('Modified products data to store: '.print_r($modified_data,1));
    ...
    private function storeModifiedFormats($modified_data, $sync_params){
        ...
        $this->slDebuger->debug('Total count of modified product formats to store: '.$product_formats_to_sync_count);
        if ($this->sl_DEBBUG > 1) $this->slDebuger->debug('Modified product formats data to store: '.print_r($modified_data,1));
    ...
    private function updateConns($sl_multiconn_reg, $item_type, $comp_id, $item_id, $sl_item_connectors){
        ...
        if ($sl_multiconn_reg['item_type'] == $item_type && $sl_multiconn_reg['sl_comp_id'] == $comp_id && $sl_multiconn_reg['sl_id'] == $item_id){
            ...
            try{
                ...
                $connectors_data = $this->slJson->unserialize($sl_multiconn_reg['sl_connectors']);

                if (!is_array($connectors_data) || (is_array($connectors_data) && empty($connectors_data))){ $connectors_data = array(); }

                $new_connectors_data = $this->slJson->serialize(array_unique(array_merge($connectors_data, $sl_item_connectors)));
                ...
                $this->slDebuger->debug('## Error. Updating multiconn table: '.$e->getMessage());
                ...
                foreach ($sl_item_data as $item_id => $sl_item_connectors) {
                    ...
                    if (!$found){
                        ...
                        try{
                            ...
                            $connectors_data = $this->slJson->serialize($sl_item_connectors);
                            ...
                            $this->slDebuger->debug('## Error. Inserting multiconn table: '.$e->getMessage());
    ...
    private function prepareConnTableData($data_tabla, $nombre_tabla, $sl_data, $comp_id, $connector_id){
        ...
        switch ($nombre_tabla) {
            ...
            default:
                ...
                $this->slDebuger->debug('## Error. Updating multiconn table, table '.$nombre_tabla.' not recognized.');
