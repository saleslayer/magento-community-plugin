    ...
    private function checkExistingImagePosition($mg_image_id, $mg_image_position, $sl_image_position){
        ...
        if ($mg_image_position != $sl_image_position){
            ...
            $this->slConnection->slDBUpdate(
                ...
                'Updating image position with ID: '.$mg_image_id.', message'
            );
    ...
    public function sync_stored_product_images_db($images_data, $item_index)
    {
        ...
        if (isset($images_data['final_images']) && !empty($images_data['final_images'])) {
            ...
            foreach ($images_data['final_images'] as $image_filename => $image_info) {
                ...
                $this->processImage($item_id, $image_filename, $image_info['url'], $image_info['media_attribute'], $image_info['position'], $image_file_size);
            ...
            if (null === $this->mg_product_attribute_set_id) {
                ...
                foreach($this->format_additional_fields as $field_name => $field_name_value) {
                    ...
                    if (!isset($this->attributeCodesByAttributeSetId[$this->mg_product_attribute_set_id])){
                        ...
                        if ($this->sl_DEBBUG > 2) { $this->slDebuger->debug('# time_load_attribute_codes_by_attribute_set_id: ', 'timer', (microtime(1) - $time_ini_load_attribute_codes_by_attribute_set_id));
                        }
                    }
